...
def main():
    ...
    '''
    # train
    network, test_fn, est_fn = utils.cnn_train(inputs_train=x_train, targets_train=y_train,
                              inputs_val=x_val, targets_val=y_val,
                              network = None,
                              batchsize=100, num_epochs=numepoch)
    # test
    utils.cnn_test(inputs=x_test, targets=y_test, network=network, test_fn = test_fn, batchsize=100)
    # save model
    savepath = os.path.join(outpath,'model.pkl')
    savedict = {'network':network, 'est_fn':est_fn}
    utils.cnn_save(savepath=savepath, savedict=savedict)
    # save params
    parampath = os.path.join(outpath, 'params.mat')
