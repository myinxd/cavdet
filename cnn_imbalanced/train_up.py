...
"""
Load data, oversampling on minority class,and train the CNN network.

Note
====
Model and parameters are saved as 'model_up.pkl' and 'params_up.mat'
"""
...
def main():
    ...
    ratio_cav = bkg_label.shape[0] // cav_label.shape[0]
    ratio_ext = bkg_label.shape[0] // ext_label.shape[0]
    print("Training on the upsampled dataset")
    data_train = np.row_stack((bkg_data,
                               np.tile(cav_data,(ratio_cav,1,1,1)),
                               np.tile(ext_data,(ratio_ext,1,1,1))))
    label_train = np.row_stack((bkg_label,
                                np.tile(cav_label,(ratio_cav,1)),
                                np.tile(ext_label,(ratio_ext,1))))
