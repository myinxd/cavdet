...
def cnn_train(inputs_train, targets_train, inputs_val, targets_val,
              network=None,
              sample_imp=[1.0, 1.0, 1.0],
              batchsize=100, num_epochs=100):
    """
    ...
    imp_var = T.dmatrix('sp_imp') # sample importance
    ...
    prediction_imp = prediction*imp_var # sample importance function
    loss = lasagne.objectives.categorical_crossentropy(prediction_imp, target_var)
    loss = loss.mean()
    ...
    val_imp = val_prediction*imp_var # sample importance function
    val_loss = lasagne.objectives.categorical_crossentropy(val_imp,
                                                           target_var)
    val_loss = val_loss.mean()
    val_acc = T.mean(T.eq(T.argmax(val_imp, axis=1), target_var),
                     dtype=theano.config.floatX)

    # Create the loss expression for estimation
    est_prediction = lasagne.layers.get_output(network, deterministic=True)
    label_est = T.argmax(est_prediction*imp_var, axis=1)

    # Compile the train function
    train_fn = theano.function([input_var, target_var, imp_var], loss, updates=updates)
    # Compile the validation function
    val_fn = theano.function([input_var, target_var, imp_var], [val_loss, val_acc])
    # Compile the estimation function
    est_fn = theano.function([input_var, imp_var], [label_est])

    # Training
    print("Starting training...")

    imp_mat = np.tile(sample_imp,(batchsize, 1))
    imp_mat.shape
    for epoch in range(num_epochs):
        ...
        for batch in iterate_minibatches(inputs_train, targets_train, batchsize, shuffle=True):
            ...
            train_err += train_fn(input_batch, target_batch, imp_mat)
            train_batches += 1
        ...
        for batch in iterate_minibatches(inputs_val, targets_val, batchsize, shuffle=True):
            input_batch, target_batch = batch
            err, acc = val_fn(input_batch, target_batch, imp_mat)
            val_err += err
            val_acc += acc
            val_batches += 1
...
def cnn_test(inputs, targets, network, test_fn, sample_imp=[1, 1, 1],batchsize=100):
    """
    ...
    for batch in iterate_minibatches(inputs, targets, batchsize, shuffle=True):
        input_batch, target_batch = batch
        err, acc = test_fn(input_batch, target_batch, imp_mat)
        test_err += err
        test_acc += acc
        test_batches += 1
...
def cnn_estimate(inputs, network, est_fn, sample_imp=[1, 1, 1],):
    """
    ...
    # estimate
    # print("Estimating ...")
    imp_mat = np.tile(sample_imp,(inputs.shape[0], 1))
    label_est = est_fn(inputs, imp_mat)
